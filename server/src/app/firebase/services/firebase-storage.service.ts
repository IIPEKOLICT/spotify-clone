...
export class FirebaseStorageService {
  private static readonly DEFAULT_EXPIRATION = 86400;

  private readonly bucket: Bucket;

  constructor(private readonly firebaseService: FirebaseService) {
    this.bucket = firebaseService.app.storage().bucket();
  }

  async getLink(path: string, expiration = FirebaseStorageService.DEFAULT_EXPIRATION): Promise<string> {
    const date = new Date();
    date.setSeconds(date.getSeconds() + expiration);

    const response: GetSignedUrlResponse = await this.bucket.file(path).getSignedUrl({ action: 'read', expires: date });
